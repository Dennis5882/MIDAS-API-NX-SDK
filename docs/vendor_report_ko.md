...
| # | 대상 | 내용 | 심각도 |
| --- | --- | --- | --- |
| A-1 | `/db/NMAS` | **POST 1회로 프로그램이 종료됨. CIVIL·GEN 양쪽 모두 재현, 11회 전부 재현** | 치명적 |
...
- **CIVIL NX에서 8회 / 3개 버전(v2.1 build 06/05, v2.2 build 06/18, v2.2 build 07/28)에서
  100% 재현**됩니다 — 새로 설치한 최신 빌드에서도 고쳐지지 않았습니다.
- **GEN NX v2.1 (build 07/28/2026)에서도 3회 전부 재현**됩니다 — 오류 문자열은 시도마다
  조금씩 달랐지만(즉시 `404 Client Disconnected` 1회, 호출 자체가 15초 타임아웃 2회) 결과는
  매번 동일했습니다: 대상 호출 이후 세션 전체가 응답하지 않습니다.
- **세 번째 GEN 재현은 합성 테스트 데이터가 아니라 실제 실무 모델에서 이루어졌습니다.**
  기존 모델의 절점은 전혀 건드리지 않고 신규 절점(9001 이상)만 사용했는데도 동일하게
  재현됐습니다 — "합성 데이터에서만 발생"이라는 가설을 배제합니다. 두 제품이 공유하는
  쓰기 경로의 결함으로 보이며, CIVIL NX 한정 문제가 아닙니다.
- **모델 형상이 원인이 아닙니다.** 지금까지의 재현은 모두 절점/요소가 여러 개 딸린
  모델이었기 때문에, "구속되지 않고 떠 있는 절점이나 부분구조가 있어서 질량행렬 갱신 시
  특이(singular) 상태가 되어 멈추는 것 아니냐"는 대안 가설을 검토했습니다. 이를 배제하기
  위해 **절점 2개 + 보 요소 1개 + 고정단 지점 1개뿐인, 부유 요소가 전혀 없는 최소 구조
  모델**(캔틸레버 기둥 1개)을 새로 만들어 자유단 절점에 동일한 페이로드로 재현했더니
  **동일하게 종료**됐습니다. 구조적으로 완전한 모델에서도 재현되므로, 모델의 기하학적
  결함이나 특이 강성행렬 때문이 아니라 `/db/NMAS`의 쓰기 처리 자체의 결함입니다.
- `GET /db/NMAS`와 `GET /info/db/NMAS`는 정상입니다. 쓰기 경로 한정으로 보입니다.
- 페이로드는 평범한 절점 1개에 단위질량 3개이며 `/info/db/NMAS` 스키마와 일치합니다.

조사 범위를 좁히시는 데 참고가 될 만한 사항으로, 아래 세 가지는 배제했습니다.
...
- **모달 대화상자 차단 아님** — `/doc/NEW`를 호출하지 않는 구성으로도 재현됩니다. 직전
  ...
  이들도 함께 멈췄어야 합니다.
- **모델 결함(부유 절점·미구속 부분구조) 때문도 아닙니다** — 위 최소 재현 참고.
