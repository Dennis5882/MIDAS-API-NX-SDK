...
| `/db/CONS` | `CONSTRAINT` 8자 | 앞 7자로 **절단** → 요청하지 않은 구속이 생성됨 | 200, 오류 없음 |
| `/db/MVHL` | `VEH_DEFAULT: {}` | 저장되지 않음 | `{"message": ""}` |
...
- A-1, A-3(`/db/SECF`, `/db/MVHL`), A-4, A-6, B-1~B-8은 **v2.2 (build 06/18/2026)**
  에서 확인했습니다. A-1은 v2.1에서도 확인했습니다.
- A-2, A-3의 `/db/CONS`·`VEH_DEFAULT` 항목은 **v2.1 (build 06/05/2026)** 세션에서
  확인한 것으로, v2.2 재확인은 아직 하지 않았습니다.
