...
| 버전 | **2026-09-21에 두 제품(Gen NX 2026 v2.1, Civil NX 2026 v2.2)에서 A-2·A-3·A-4·A-6·A-8·A-9·A-10을 일괄 재확인**했습니다. 그날 `/info` 전수 스캔 결과가 Build 09/15/2026 기준 스냅샷과 일치합니다 |
| 재확인 안 된 항목 | A-5(제품 종료가 필요)와 A-7(Program Files 하위 문서 필요)만 남았으며, 각각 2026-07과 2026-07-29가 마지막 측정입니다 |
...
### 2026-09-21 재측정 — 두 제품이 갈렸고, 원인이 좁혀졌습니다

폐기용 더미 모델을 만들어 `POST /doc/SAVEAS`로 `C:/Program Files/` 아래에 직접
저장을 시도했습니다. **같은 릴레이·같은 호출·같은 경로 형태인데 결과가 반대입니다.**

| 제품 | 결과 |
| --- | --- |
| CIVIL NX | `{"message": "... command complete"}`. 이어진 `/doc/OPEN`이 **성공** |
| GEN NX | **HTTP 응답이 오지 않음**(60초 타임아웃), 세션 차단 |
...
첫째, 차단된 동안 `/mapikey/verify`는 계속 **1초 이내에 `connected`**를 반환했습니다
(수 분 간격 2회). A-5와 같은 현상이며, 이번에는 크래시가 아니라 모달 대기 상태에서
관측됐습니다 — 릴레이가 응답하므로 제품이 멈춘 것을 볼 수 없습니다.

둘째, **이 실패 형태에서는 `/doc/SAVEAS`가 "command complete"를 반환하지 않았습니다.**
호출 자체가 대화상자를 닫을 때까지 응답하지 않았습니다. 존재하지 않는 경로를 줬을 때
성공처럼 답하는 기존 사례(2026-07-26)와는 다른 동작이라 구분해서 적습니다.

**그리고 Civil에서는 재현되지 않았습니다.** Program Files 아래 문서를 열어 둔 채
`GET /db/CAMB`를 호출해도 `{"message": ""}`가 정상 반환되고 세션도 멀쩡했습니다.
즉 이 문제의 방아쇠는 **경로가 `Program Files`라는 것이 아니라, 그 경로에 대한 쓰기가
실패하는 것**입니다. 쓰기가 통과하면 조회성 호출에서도 아무 일이 없습니다.

Civil에서 쓰기가 통과한 이유는 저희 쪽에서 특정하지 못했습니다. 제품이 상승된 권한으로
실행 중이거나, Windows의 UAC 가상화가 쓰기를 `%LOCALAPPDATA%\VirtualStore\` 아래로
돌렸을 가능성이 있습니다. **후자라면 더 나쁜 사례입니다** — 저장이 성공으로 보고되고
`/doc/OPEN`까지 성공하지만, 사용자가 지정한 위치에는 파일이 없습니다. 어느 쪽인지
확인해 주시면 감사하겠습니다.
