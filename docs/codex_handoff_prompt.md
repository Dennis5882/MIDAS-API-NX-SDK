...
```bash
python -m pytest -q                       # 1047 passed, 1 FAILED - EXPECTED
...
`docs/unmerged_tables_against_info.md` splits the 75 tables (507 field names)
that 19 contracts declare missing:

| what the measurement found | tables |
| --- | ---: |
| whole table declared, **one `/info` object holds it** | 35 |
| whole table declared, several objects | 3 |
| whole table declared, no common parent | 25 |
| partly declared | 1 |
| outside `/info`'s reach (`/view`, `/ope`) | 11 |

**Your remaining part is the 35.** The first three-table batch merged
...
on `PARTS`; npm now generates IMPF from the contract too. For each remaining
