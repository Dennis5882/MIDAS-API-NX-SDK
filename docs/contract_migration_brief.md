...
| --- | ---: | ---: | --- |
| endpoint contracts | 251 | — | `post-table.yaml` is the shared-table family contract |
| npm DB resources | 231 | 304 | 73 resources still have no committed endpoint contract |
| `/post/TABLE` result tables | 87 | 87 | Chapter 23's other two routes are `/post/PM` and `/post/STEELCODECHECK`, not `TABLE_TYPE` tables |
| active promotion candidates | 0 | 132 drafts | each remaining draft has a measured gate refusal |

The remaining 73 npm resources are not a uniform parser backlog. They include
