...
| TypeScript/npm SDK | `packages/typescript/` — ESM + CommonJS + declarations, Node.js 18+, Vitest/typecheck/build and packed-artifact smoke tests | ✅ npm `midas-nx` 2.9.0 published 2026-09-20; `js-v*` OIDC workflow and npm Trusted Publisher registration completed 2026-08-27. Versions have moved in lockstep with PyPI since 2.6.0 |
...
| npm type provenance | `scripts/report_npm_type_provenance.py --check`, own CI step | 🚧 **478 of 765 generated npm types still come from the Python source tree**, held as a ceiling that can fall but not rise. **463 of them are nested objects** — the generator emits a contract's payload root and leaves everything under it to Python — 13 are the `unmergedTables` contracts, skipped on purpose, and 2 no contract names. So deleting the Python dependency is one missing generator capability plus a decision about where a nested type's published npm name lives, not 478 separate gaps |
