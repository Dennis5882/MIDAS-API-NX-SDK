...
| npm type provenance | `scripts/report_npm_type_provenance.py --check`, own CI step | 🚧 **250 of 765 generated npm types still come from the Python source tree**, held as a ceiling that can fall but not rise. Contracts now own nested types through `surface.nestedTypes`, which moved 228 named nested interfaces off Python on 2026-09-21. Most of what is left sits under no contract-generated root: `/doc`/`/ope`/`/view` operation arguments, the 13 `unmergedTables` roots and their children, and table result types; the rest are contract gaps the script's docstring lists |
