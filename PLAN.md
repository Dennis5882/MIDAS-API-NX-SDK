...
> Last updated: 2026-09-06 — **2.8.0 published** to PyPI and npm. The npm
> surface stops requiring two mutually exclusive fields on three moving-load
> control payload types; `src/midas_nx/` has no commit since 2.7.9, so PyPI
> republishes an identical wheel under the aligned number. Five more endpoints
> earned write evidence, taking coverage **185 → 190** and npm live evidence
> **55 → 60**.
>
...
| Axis | Artifact | State |
|---|---|---|
| Tests | 1026 Python tests + 75 Vitest tests, mocked/local only | ✅ green, with one contract-to-manual test failing on purpose: the sibling manual repo has an unreflected 2026-09-06 sync the author is still verifying |
...
| TypeScript/npm SDK | `packages/typescript/` — ESM + CommonJS + declarations, Node.js 18+, Vitest/typecheck/build and packed-artifact smoke tests | ✅ npm `midas-nx` 2.8.0 published 2026-09-06; `js-v*` OIDC workflow and npm Trusted Publisher registration completed 2026-08-27. Versions have moved in lockstep with PyPI since 2.6.0 |
...
| Live verification | `scripts/live_smoke.py` (write round trip), `scripts/live_readonly_sweep.py` (GET breadth) | ✅ 399/399 recorded, split by `level`: **191 write / 208 read / 0 unverified** as of 2026-09-06. Write level means a call changed model data or wrote a host file; read includes route/schema checks and POST-shaped reads. ⚠️ **The build baseline is currently self-contradictory**: entries dated 2026-09-06 record Gen NX v2.1 / Civil NX v2.2 at both Build 09/02/2026 and Build 08/26–08/27/2026. A build cannot go backwards, so one set is wrong, and **the API reports no build** — every such string is a human reading the product's About dialog. Needs the author to say which is installed. |
