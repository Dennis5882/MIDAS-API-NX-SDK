...
├── endpoints/                    one YAML per endpoint — 283 endpoint contracts so far
...
| Axis | Artifact | State |
|---|---|---|
| Tests | 863 Python tests + 55 Vitest tests, mocked/local only | ✅ green |
...
| Language-neutral contracts | `contracts/` + `scripts/{extract,promote,validate}_contract*.py`, own CI job | 🚧 **283 endpoints + 87 result tables**, 2,228 fields, with **101** drafts awaiting review (`extract_contracts.py --report`, not raw ignored draft files). Drafted from the manual by `extract_contracts.py`, promoted by `promote_contract.py`. Validates schema, cross-references, safety-rule coverage, manual drift, and **parity against both SDKs** — a disagreement is an SDK defect, not a reason to edit the contract. It has caught: npm able to crash a live NX session on `/db/NMAS`; `/db/GRUP` claiming a DELETE it does not serve; `/db/RIGD`/`/db/OFFS` flattening an `ITEMS` array; and 7 endpoints wrongly called Civil-only |
...
| Live verification | `scripts/live_smoke.py` (write round trip), `scripts/live_readonly_sweep.py` (GET breadth) | ✅ 399/399 recorded, split by `level`: **165 write / 234 read / 0 unverified** as of 2026-08-31. Write level means a call changed model data or wrote a host file; read includes route/schema checks and POST-shaped reads. The current build baseline is Gen NX 2026 v2.1 and Civil NX 2026 v2.2, both Build 08/26/2026. |
