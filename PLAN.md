...
| TypeScript/npm SDK | `packages/typescript/` — ESM + CommonJS + declarations, Node.js 18+, Vitest/typecheck/build and packed-artifact smoke tests | ✅ npm `midas-nx` 2.7.8 published 2026-09-05; `js-v*` OIDC workflow and npm Trusted Publisher registration completed 2026-08-27. Versions have moved in lockstep with PyPI since 2.6.0 |
...
| Fixture ↔ contract agreement | `scripts/check_fixture_contract.py --check`, own CI step | 🚧 **new 2026-09-05**, and the fourth artefact claiming to know an endpoint's shape was the one nothing checked. 118 disagreements, split by whether the case has ever passed live: **54 across 8 endpoints** where the payload is the suspect (`/db/ACTL` sending a Civil-only field on Gen; `/db/MVCT`/`/db/NLNK`/`/db/NLNK-M1`/`/db/TDMF` omitting required fields), and **64 across 18 endpoints on `confirmed` cases**, which read the opposite way — the product accepted that payload, so the contract is behind. Held in CI in both directions; nothing merged, because a fixture is never a source for a contract |
