...
| npm type provenance | `scripts/report_npm_type_provenance.py --check`, own CI step | 🚧 **15 of 765 generated npm types still come from the Python source tree**, held as a ceiling that can fall but not rise. Since 2026-09-21/22 contracts own nested payload types (`surface.nestedTypes`), the operation and table-wrapper lists (operation `surface` and table `surface`), operation argument types with their nested types, each table's own request objects (`requestFields.additional`), and where every contract-built type is placed - Python supplies nothing for a type a contract owns. What is left: the load-combination AIK-SRC2K part the manual types differently per route (the 0 `unmergedTables` roots left since 2026-09-22 add nothing), one shared `/post` type the manual makes required in two tables and optional in two, /db names whose contracts genuinely disagree, and Python base classes with no wire object - the provenance script's docstring lists all 13 nested ones |
