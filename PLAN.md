...
| npm type provenance | `scripts/report_npm_type_provenance.py --check`, own CI step | 🚧 **162 of 765 generated npm types still come from the Python source tree**, held as a ceiling that can fall but not rise. Since 2026-09-21/22 contracts own nested payload types (`surface.nestedTypes`), the operation and table-wrapper lists (operation `surface` and table `surface`), and operation argument types with their nested types. What is left: argument types held back for a stated reason, the 13 `unmergedTables` roots and their children, table result types, and contract gaps the provenance script's docstring lists |
